...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Incorrect number of arguments passed.')
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    check_form(expressions, 2)
    eval_op = expressions.first
    if isinstance(eval_op, Pair) and scheme_symbolp(eval_op.first):
        check_formals(eval_op.second)
        env.define(eval_op.first, MacroProcedure(eval_op.second, expressions.second, env))
        return eval_op.first
    else:
        raise SchemeError
